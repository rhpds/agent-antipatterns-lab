...
|---------|-------|----------|
| 1 | Describe what the agent can reach | 2 min |
| 2 | Point the agent at the gateway | 5 min |
| 3 | See the governed catalog | 2 min |
...
- **Agent identity is a different axis and is deliberately absent.** This module is about caller identity deciding which tools the agent may reach. It is not about the agent's own cryptographic identity, meaning who the agent claims to be when another agent calls it. That second axis is moving fast: A2A Agent Cards, SPIFFE and SPIRE workload binding via the `spiffe://<trust-domain>/ns/<namespace>/sa/<serviceAccount>` convention, and JWS-signed cards. Red Hat's work on it now lands in OpenShell, named in the agentic strategy as the unified agent security and lifecycle project, with an operator planned for RHOAI 3.6 to handle identity injection. The neighbouring Security/Governance lab is built on OpenShell and lists SPIFFE and SPIRE in its product set, so adopting agent cards here would rebuild the exact overlap that got the previous design rejected. Checked 2026-09-02: no Kagenti, SPIRE or A2A component exists in any catalog source on the dev cluster.
...
- **Two other assets touch MCP and were checked on 2026-09-02.** The published *vLLM Playground* lab has an MCP integration module, and `showroom-agentic-ai-llamastack` has a dedicated MCP module. Both connect an agent to MCP servers. This module's distinct claim is narrow and should stay narrow: not that MCP exists or how to connect to a server, but that routing tool access through a governed endpoint makes the agent's reach a describable property of the platform rather than of its source code. If that distinction cannot be made clearly in 10 minutes, the module is better cut than blurred.
