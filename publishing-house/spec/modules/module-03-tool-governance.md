...
- MCP Gateway is Tech Preview and Red Hat provided, verified as operator v0.7.1 on the dev cluster. Tech Preview is explicitly not covered by Red Hat production SLAs. Documented fallback if it regresses before the event: tool scoping at the MCP server or registry level plus RBAC — same principle, no gateway dependency.
