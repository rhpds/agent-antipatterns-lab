...
- **Section 5 must be authored as a separable beat, and this is now load-bearing rather than precautionary.** The maturity question was settled on 2026-08-19 and the answer was unfavourable: EvalHub is Technology Preview in **both** RHOAI 3.4 and 3.5 — client SDK/CLI and Evaluation Stack UI alike — with no announced GA date, so it will almost certainly still be Tech Preview at the event. Garak's own tier is ambiguous, documented as a Tech Preview EvalHub provider but described as developer preview in OpenShift AI 3.4 in Red Hat blog material. **Author decision 2026-08-19: keep Garak as Round 4's verification instrument and disclose Tech Preview status to participants rather than mitigate it.** That makes the MLflow judge fallback in step 19 the round's insurance policy, and it must be a genuine drop-in substitution requiring no restructuring — verify this during authoring rather than assuming it.
- **A distribution rename is in flight and must be re-verified before the event.** The Garak provider ships in the Llama Stack distribution in 3.4 and in the OGX distribution in 3.5. This lab reaches Garak through the TrustyAI EvalHub CR rather than through that distribution, which is why it ran on a dev cluster with `llamastackoperator` set to Removed — but the packaging is moving underneath the round, and it needs re-verification against whichever RHOAI version ships at Red Hat One.
- **Round 4's assessed outcome does not depend on any of this.** The learning objective is verified by the Guardrails Orchestrator returning a blocked verdict for every adversarial case, and the orchestrator has been GA since RHOAI 2.19. A Garak or EvalHub regression costs the quantified Attack Success Rate — the thing that makes the result measurable rather than anecdotal — but does not invalidate the round.
