...
6. Inspect the serving arguments on both class endpoints. Both set `--enable-auto-tool-choice`. Both set `--reasoning-parser=qwen3`, `--dtype=auto`, `--max-model-len=32768`, `--gpu-memory-utilization=0.90` and `--seed=0` identically.
...
- **Both endpoints must serve with greedy decoding and a fixed seed.** The parser mismatch is structural, but the evidence a participant reads depends on the model emitting a tool call for the input. Fixing sampling makes the response identical for all thirty participants rather than sampled thirty times, which is what the design's "never dependent on the model choosing" rule requires in practice. Verify by running the module 1 input repeatedly against the broken endpoint before authoring completes.
