...
- **Vector store selection is the one open item in this design.** The requirement is modest: a searchable corpus, source identifiers on results, and enough throughput for 30 concurrent participants issuing short queries. Selection should favour whatever RHOAI already ships or that GitOps can stand up without adding an operator dependency. Flagged for the infra reviewer, and to be settled against the dev cluster before authoring begins.
...
- **Scope discipline.** This module deliberately does not tune retrieval quality, chunking, or embedding choice. LB1782 in the same topic slot builds a full RAG pipeline over enterprise documentation; going deeper here would duplicate it. The objective is a grounded agent, not a tuned retriever.
