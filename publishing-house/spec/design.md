...
Participants build the agent themselves. In each module they add a capability, run it against a realistic input, watch it fail in a way that produces no error and no obvious symptom, locate the cause in the evidence rather than in the answer, and then build the step that prevents it. The rule established in module 1 and held throughout is that the agent's answer is a claim and something else is the evidence.

**Which evidence, and when.** Modules 1 and 2 read the raw request and response payloads on the inference endpoint, because that is the only place their failure is visible: the model emitted a correct tool call, nothing parsed it, and it sits in the response body as text. No trace view surfaces that. From module 3 onward the evidence is the execution trace, which is the better instrument for turn counts, tool sequences and error categories. The habit is checking evidence over answers; the instrument changes with the failure.
